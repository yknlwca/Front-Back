...
export default function TopBar() {
  ...
  const switchLanguage = () => {
    ...
    setTostMsg(i18n.language === 'kr' ? '한국어로 전환' : 'Switch to English')
    if (TostTimer != null) {
      setShowToast(false)
            ...
            <label htmlFor='infoModal' className="modal h-screen">
              ...
              >
                <div>
                  ...
                  <span className=' text-lg'>{t('room.isAdmin') + ": " + (mcurState.isAdmin ? "예" : "아니요")}</span>
                  <br />
                  <span className=' text-lg'>{t('room.passwd') + ": " + (roominfo_after_enter.passwd ? roominfo_after_enter.passwd : "비밀번호 없음")}</span>
                  <br />
                </div>
              </label>
            </label>
