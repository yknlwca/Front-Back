...
export function OptionPanel({ showIcon, showText, ...props }: any) {
  ...
  const denoiseSetting = useObservableState(denoiseMethod$, {
    ...defaultAudioSetting.denoiseMethod,
  });
  const mcurState = useCurState();
  const [passwd, setPasswd] = useState("");
  const [capacity, setCapacity] = useState("");
  const [config, setConfig] = useState<DenoiseMethod>({
    ...defaultAudioSetting.denoiseMethod,
  });
  ...
  const handleSubmit = (e: any) => {
    if (
      roominfo_after_enter.room_name == undefined &&
  ...
  return (
    <div className="flex text-center justify-center items-center">
      <label
        htmlFor="optionModel"
        className="btn border-none btn-primary text-white gap-1"
      >
        {showIcon && <SettingIcon />}
        {showText && t("setting.setting")}
      </label>
        ...
        <div className="modal-box relative bg-primary form-control">
          ...
          {mcurState.isAdmin && (
            <div className="pl-4 pr-12">
              <label className="label cursor-pointer justify-between">
                <span className="label-text text-white sm:text-lg">
                  {t("setting.passwd")}
                </span>
                <input
          ...
          {isMainBrowser && (
            <div>
              ...
              <div
                className="my-2 w-full flex justify-around"
                onChange={(v: any) => {
              ...
              >
                <div className="flex items-center gap-2 text-center">
                  <input
                    ...
                    checked={
                      config
                        ? config.speex == false && config.rnn == false
                        : false
                    }
                    onChange={(v) => {}}
                  />
                  <label htmlFor="denoiseMethod1">없음</label>
                </div>
                ...
                <div className="flex items-center gap-2 text-center">
                  <input
                    ...
                    id="denoiseMethod2"
                    className="radio radio-success"
                    checked={config ? config.speex : false}
                    onChange={(v) => {}}
                  />
                ...
                <div className="flex items-center gap-2 text-center">
                  <input
                    ...
                    id="denoiseMethod3"
                    className="radio radio-success"
                    checked={config ? config.rnn : false}
                    onChange={(v) => {}}
                  />
          ...
          <div className="w-full flex justify-center">
            <label
              htmlFor="optionModel"
              className="btn btn-md btn-secondary border-none mt-2"
